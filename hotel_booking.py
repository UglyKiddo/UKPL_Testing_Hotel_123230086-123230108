"""
╔══════════════════════════════════════════════════════════════╗
║           HOTEL NUSANTARA - BOOKING SYSTEM v1.0             ║
...
# ─────────────────────────────────────────────
#  RIWAYAT BOOKING (disimpan selama program berjalan)
# ─────────────────────────────────────────────
booking_history = []
booking_counter = 1
...
def input_angka(prompt, min_val=None, max_val=None):
    """Meminta input angka bulat dengan validasi lengkap."""
    while True:
        try:
            ...
            print("  ⚠  Masukkan angka yang valid (tanpa huruf/simbol).\n")


def input_pilihan_menu(prompt, pilihan_valid):
    """Meminta pilihan menu, hanya menerima nilai dalam daftar valid."""
    while True:
        raw = input(prompt).strip()
        if raw == "":
            print("  ⚠  Input tidak boleh kosong.\n")
...
def login():
    ...
    header("LOGIN")
    print("  Masukkan kredensial Anda untuk masuk.\n")
    print("  (Demo akun: admin/admin123 | budi/budi456)\n")
    garis("─")
    print()
    ...
    print("\n  ⛔ Terlalu banyak percobaan. Program dihentikan.")
    exit()
...
def booking_kamar(username):
    ...
    while True:
        ...
        if bayar < total:
            kurang = total - bayar
            print(f"\n  ❌ Pembayaran kurang {format_rupiah(kurang)}. Silakan masukkan ulang.\n")
        else:
            break
    ...
    print("              Kami berharap Anda menikmati menginap.")
    garis("═")
...
def checkout(username):
    """Proses checkout dan melihat riwayat booking."""
    header("CHECKOUT & RIWAYAT BOOKING")

    # Filter booking milik user yang sedang login
    ...
    print()
    pilihan_str = [str(i) for i in range(1, len(my_bookings) + 1)]
    pilihan_str.append("0")

    print("  Pilih nomor untuk melihat detail / checkout.")
    print("  [0] Kembali ke menu utama\n")

    nomor = input_pilihan_menu(
        f"  Pilihan [0-{len(my_bookings)}] : ", pilihan_str
    )
    ...
    if booking["status"] == "Aktif":
        ...
        print("\n  ℹ  Booking ini sudah selesai / di-checkout.\n")
...
def menu_utama(username):
    ...
    while True:
        header(f"MENU UTAMA  │  Pengguna: {username.capitalize()}")
        print("  Selamat datang! Pilih menu di bawah ini:\n")
...
def main():
    while True:
        ...
        # Tanya apakah ingin login lagi
        clear_screen()
        garis()
        print("  Login dengan akun lain?")
        garis("─")
        lagi = input_pilihan_menu("\n  [y] Login lagi  |  [n] Keluar program  → ", ["y", "Y", "n", "N"])
        if lagi.lower() == "n":
            clear_screen()
            garis()
